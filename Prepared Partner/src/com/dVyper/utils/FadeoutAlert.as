package com.dVyper.utils {
	...
	public class FadeoutAlert {
		...
		private static var actualPrompt:Sprite;
		...
		*/
		...
		public static function show(Text:* = "Made by dVyper", ALERTOPTIONS:Object = null):void {
			...
			myAlert.blendMode         = BlendMode.LAYER; // needed for alpha of text
		...
		private static function beginFade(e:Event):void {
			_fadeout = new Timer(100, 20); // Fade timer
			_fadeout.addEventListener(TimerEvent.TIMER, fadeOut);
			_fadeout.addEventListener(TimerEvent.TIMER_COMPLETE, fadeComplete);
			_fadeout.start();
			trace("Beginning fade...");
		}
		private static function fadeOut(e:TimerEvent):void {
			myAlert.alpha -= 0.05; //event.target.parent
			trace("Fade: " + myAlert.alpha);
		}
		...
		private static function fadeComplete(e:TimerEvent):void {
			trace("...Fade complete.");
			closeAlert(myAlert);
		}
		//
		...
		private static function closeAlert(myAlert:Sprite):void {
			_fadeout.removeEventListener(TimerEvent.TIMER_COMPLETE, fadeComplete);
			_fadeout.removeEventListener(TimerEvent.TIMER, fadeOut);
			actualPrompt.removeEventListener(MouseEvent.ROLL_OVER, mouseOver);
			actualPrompt.removeEventListener(MouseEvent.ROLL_OUT, beginFade);
			_fadeout.stop();
		...
		private static function getPrompt(alertOptions:AlertOptions):Sprite {
			actualPrompt = createPrompt(alertOptions);
			...
			actualPrompt.addEventListener(MouseEvent.ROLL_OVER, mouseOver);
			actualPrompt.addEventListener(MouseEvent.ROLL_OUT, beginFade);
